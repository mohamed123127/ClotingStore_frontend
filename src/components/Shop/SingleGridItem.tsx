...
const SingleGridItem = ({ item }: { item: Product }) => {
  ...
  const handleItemToWishList = () => {
    // console.log(wishlistItems.find(w => w.id === item.id));
    if(wishlistItems.find(w => w.id === item.id) == undefined){
      dispatch(
        addItemToWishlist({
          ...item
        })
  ...
  return (
    <div className="group">
      <div className="relative overflow-hidden flex items-center justify-center rounded-lg bg-white shadow-1 min-h-[270px] mb-0">
        ...
        <Image src={item.previewImage} alt="preview" sizes="(max-width: 768px) 100vw, 250px" fill 
          className="object-cover cursor-pointer" onClick={() => {
          openModal();
        ...
        <div className="absolute left-0 bottom-0 translate-y-full w-full flex items-center justify-center gap-2.5 pb-5 ease-linear duration-200 group-hover:translate-y-0">
          ...
          <button
            ...
            id="favOne"
            className={`flex items-center justify-center w-9 h-9 rounded-[5px] shadow-1 ease-out duration-200 ${wishlistItems.find(w => w.id === item.id) ? 'text-red' : 'text-dark'} bg-white hover:text-blue`}
          >
            <svg
              className="fill-current"
              width="16"
      ...
      <span className="flex items-center gap-2 font-medium text-lg">
        <span className={`text-dark ${item.discountedPrice != null ? '' : 'hidden'}`}>{item.discountedPrice}DA</span>
        <span className={`${item.discountedPrice != null ? 'text-dark-4 line-through' : 'text-dark'}`}>{item.price}DA</span>
      </span>
    </div>
  );
};
